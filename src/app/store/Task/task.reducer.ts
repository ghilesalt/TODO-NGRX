...
export const taskReducer = createReducer(
  ...
  on(TaskActions.deleteTask, (state, { task }) => state.filter(t => t.id !== task.id)),
  on(TaskActions.updateTask, (state, { task }) => state.map(t => t.id === task.id ? task : t)),
  on(TaskActions.advanceTaskStatus, (state, { task }) => state.map(t => t.id === task.id ? { ...t, status: nextStatus(t.status) } : t))
);
